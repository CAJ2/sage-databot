    ...
    while True:
        ...
        for attempt in range(max_retries):
            try:
                        ...
                        if isinstance(hierarchy_data, list) and hierarchy_data:
                            if (
                                ...
                                for key in order:
                                    if (
                                        key in h_dict
                                        and f"wof_{h_dict[key]}" != region_id
                                    ):
                ...
                if (
                    "restart transaction" in str(e).lower()
                    and attempt < max_retries - 1
                ):
                    wait = (attempt + 1) * 2
                    print(
                        f"Serialization error, retrying in {wait}s... (attempt {attempt + 1}/{max_retries})"
                    )
                    time.sleep(wait)
                    continue
                else:
                    raise
        if not success:
            print("Failed to process batch after max retries.")
            break
