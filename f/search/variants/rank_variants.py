    ...
    with crdb.connect() as conn:
        rows = conn.execute(
            text(
                ...
                (code IS NOT NULL) AS has_code,
                ((SELECT 1 FROM public.variants_sources vs
                  JOIN public.sources s ON vs.source_id=s.id
                  WHERE vs.variant_id=variants.id AND vs.meta->>'order'='1' AND s.type='IMAGE'
                  LIMIT 1) IS NOT NULL) AS has_image,
                ((SELECT count(*) FROM public.variants_items WHERE variant_id=variants.id) > 0) AS has_items,
                ((SELECT count(*) FROM public.variants_components WHERE variant_id=variants.id) > 0) AS has_components
                FROM public.variants WHERE id IN ('{ids_join}')"""
            )
        ).fetchall()
    return {
        str(row[0]): {
            ...
            "has_items": bool(row[5]),
            "has_components": bool(row[6]),
        }
        for row in rows
    }
...
def query_text(crdb: Engine, ids: list[str]) -> dict[str, dict[str, str | None]]:
    ...
    with crdb.connect() as conn:
        rows = conn.execute(
            text(
                f"""SELECT id, name->>'en', "desc"->>'en', code
                FROM public.variants WHERE id IN ('{ids_join}')"""
            )
        ).fetchall()
    return {
        str(row[0]): {"name": row[1], "desc": row[2], "code": row[3]} for row in rows
    }
...
def main(keys: list[str]) -> dict[str, int]:
    ...
    views_by_id = query_pop("variant", keys)
    ...
    for id_ in keys:
        ...
        if new_hash == old_hash and old_score is not None:
            ...
            llm_score = score_entity("variants", text_fields, model)
