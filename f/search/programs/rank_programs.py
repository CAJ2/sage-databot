    ...
    with crdb.connect() as conn:
        rows = conn.execute(
            text(
                ...
                (SELECT COUNT(*) > 0 FROM public.program_orgs WHERE program_id = programs.id) AS has_orgs,
                (region_id IS NOT NULL) AS has_region,
                (SELECT COUNT(*) > 0 FROM public.program_tags WHERE program_id = programs.id) AS has_tags
                FROM public.programs WHERE id IN ('{ids_join}')"""
            )
        ).fetchall()
    return {
        str(row[0]): {
...
def main(keys: list[str]) -> dict[str, int]:
    ...
    views_by_id = query_pop("program", keys)
    ...
    for id_ in keys:
        ...
        if new_hash == old_hash and old_score is not None:
            ...
            llm_score = score_entity("programs", text_fields, model)
