...
WEIGHTS = {
    "has_name_en": 0.25,
    "has_desc_en": 0.20,
    "has_visual_image": 0.25,
    "has_physical": 0.15,
    "has_primary_material": 0.15,
}
    ...
    with crdb.connect() as conn:
        rows = conn.execute(
            text(
                ...
                (visual->>'image' IS NOT NULL) AS has_visual_image,
                (physical->>'mass' IS NOT NULL OR physical->>'volume' IS NOT NULL) AS has_physical,
                (primary_material IS NOT NULL) AS has_primary_material
                FROM public.components WHERE id IN ('{ids_join}')"""
            )
        ).fetchall()
    return {
        str(row[0]): {
            ...
            "has_visual_image": bool(row[3]),
            "has_physical": bool(row[4]),
            "has_primary_material": bool(row[5]),
        }
        for row in rows
    }
...
def main(keys: list[str]) -> dict[str, int]:
    ...
    views_by_id = query_pop("component", keys)
    ...
    for id_ in keys:
        ...
        if new_hash == old_hash and old_score is not None:
            ...
            llm_score = score_entity("components", text_fields, model)
