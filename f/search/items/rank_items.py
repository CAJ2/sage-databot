    ...
    with crdb.connect() as conn:
        rows = conn.execute(
            text(
                ...
                (files->>'thumbnail' IS NOT NULL) AS has_image,
                ((SELECT count(*) FROM public.items_categories WHERE item_id=items.id) > 0) AS has_categories,
                ((SELECT count(*) FROM public.items_tags WHERE item_id=items.id) > 0) AS has_tags,
                ((SELECT count(*) FROM public.variants_items WHERE item_id=items.id) > 0) AS has_variants
                FROM public.items WHERE id IN ('{ids_join}')"""
            )
        ).fetchall()
    return {
        str(row[0]): {
            ...
            "has_categories": bool(row[4]),
            "has_tags": bool(row[5]),
            "has_variants": bool(row[6]),
        }
        for row in rows
    }
...
def main(keys: list[str]) -> dict[str, int]:
    ...
    views_by_id = query_pop("item", keys)
    ...
    for id_ in keys:
        ...
        if new_hash == old_hash and old_score is not None:
            ...
            llm_score = score_entity("items", text_fields, model)
