    ...
    with crdb.connect() as conn:
        rows = conn.execute(
            text(
                ...
                (desc_short->>'en' IS NOT NULL AND LENGTH(desc_short->>'en') > 0) AS has_desc_short_en,
                (image_url IS NOT NULL) AS has_image
                FROM public.categories WHERE id IN ('{ids_join}')"""
            )
        ).fetchall()
    return {
        str(row[0]): {
...
def main(keys: list[str]) -> dict[str, int]:
    ...
    views_by_id = query_pop("category", keys)
