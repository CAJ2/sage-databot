...
def main(fix: bool = False) -> dict[str, Any]:
    """
    Verifies public.category_tree matches the closure implied by
    public.category_edges (same depth definition as
    ...
    category_tree to match the recomputed closure in the same run.
    ...
    with engine.connect() as conn:
        ...
        edge_rows = conn.execute(
            text("SELECT parent_id, child_id FROM public.category_edges")
        ).fetchall()
        tree_rows = conn.execute(
            text("SELECT ancestor_id, descendant_id, depth FROM public.category_tree")
        ).fetchall()
    ...
    if not nx.is_directed_acyclic_graph(graph):
        return {"error": "category_edges graph is not a DAG; closure is undefined."}
    if not nx.is_weakly_connected(graph):
        return {
            "error": "category_edges graph is not weakly connected; closure is undefined."
        }
    ...
    if fix and not result["clean"]:
        ...
        result["fixed"] = True
        print(
            f"Fixed category_tree: {len(missing)} missing, {len(extra)} extra, {len(wrong_depth)} wrong-depth rows repaired."
        )
    else:
        result["fixed"] = False
        if result["clean"]:
            print("category_tree matches category_edges closure.")
        else:
            print(
                f"category_tree drift: {len(missing)} missing, {len(extra)} extra, {len(wrong_depth)} wrong-depth rows."
            )
